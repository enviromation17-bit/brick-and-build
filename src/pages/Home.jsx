  ...
  return (
    <>
      ...
      >
        ...
        <Link to="/projects/pak-city" className="h-12 inline-flex items-center rounded-pill border border-white/50 text-white px-8 font-bold text-sm hover:bg-white hover:text-navy">View completed work</Link>
      </Hero>

      <div className="overflow-hidden border-y border-lineDark bg-navy py-5">
        <div className="marquee-track">
          {[0, 1].map((k) => (
            <span key={k} className="inline-flex items-center gap-10 font-display italic text-2xl text-white/80">
              {MARQUEE_ITEMS.map((m) => (
                <span key={m}>{m} <i className="text-gold not-italic">·</i></span>
              ))}
            </span>
          ))}
        </div>
      ...
      <section className="py-24">
        <div className="max-w-container mx-auto px-5 md:px-8">
          <Reveal><p className="text-[0.75rem] font-bold tracking-[0.22em] uppercase text-slate">Services</p></Reveal>
          <Reveal delay={0.05}><h2 className="mt-3 text-[clamp(1.9rem,3.6vw,3rem)] font-extrabold tracking-tight text-navy">Land. Commercial. Residential.</h2></Reveal>
          <Reveal delay={0.1}><p className="mt-4 max-w-[40ch] text-slate">Three practices, one established standard: land development, commercial and residential.</p></Reveal>
          <div className="mt-12 grid md:grid-cols-3 gap-4">
            {SERVICES.map((s, i) => (
              <Reveal key={s.title} delay={0.05 * i}><Tile n={s.n} title={s.title} body={s.body} to={s.to} img={s.img} /></Reveal>
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="py-24 bg-paper2 border-y border-line">
        <div className="max-w-container mx-auto px-5 md:px-8">
          <Reveal><p className="text-[0.75rem] font-bold tracking-[0.22em] uppercase text-slate">Projects</p></Reveal>
          ...
          <Reveal delay={0.1}><p className="mt-4 max-w-[46ch] text-slate">Pak City Housing Society is our completed, multi-site development in Pakpattan. Toba Tek Singh is our current project, underway.</p></Reveal>
      ...
      <section className="py-24 bg-navy text-white">
        <div className="max-w-container mx-auto px-5 md:px-8">
          ...
          <Reveal delay={0.05}><h2 className="mt-3 text-[clamp(1.9rem,3.6vw,3rem)] font-extrabold tracking-tight">How we approach land and development.</h2></Reveal>
      ...
      <section className="relative overflow-hidden py-28">
        ...
        <div className="relative z-10 max-w-container mx-auto px-5 md:px-8">
          ...
          <Reveal><h2 className="mt-4 max-w-[34rem] text-[clamp(2rem,4vw,3.4rem)] font-extrabold tracking-tight text-white">Pakpattan is where we're based.</h2></Reveal>
          <Reveal delay={0.1}><p className="mt-5 max-w-[28rem] text-white/80">Direct access to our team — no call centers, no sales floors.</p></Reveal>
          <Reveal delay={0.15}>
            <Link to="/contact" className="mt-8 inline-flex h-12 items-center rounded-pill bg-white text-navy px-8 font-bold text-sm hover:bg-paper2">Get in touch</Link>
          </Reveal>
        </div>
      </section>
    </>
